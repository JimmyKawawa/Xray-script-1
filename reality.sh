...
function menu() {
  ...
  if [ -d /usr/local/etc/xray-script ] && ([ ${idx} -gt 102 ] || [ ${idx} -lt 111 ]); then
    wget -qO ${xray_config_manage} https://raw.githubusercontent.com/JimmyKawawa/Xray-script-1/main/tool/xray_config_manage.sh
  ...
  1)
    if [ ! -d /usr/local/etc/xray-script ]; then
      ...
      wget -O ${xray_config_manage} https://raw.githubusercontent.com/JimmyKawawa/Xray-script-1/main/tool/xray_config_manage.sh
      chmod a+x ${xray_config_manage}
